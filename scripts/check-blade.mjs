...
// RNGドラフトを介さず直接mod_bladeを所持状態にする(適用経路自体はUIタップと同じapplyChoice)。
await page.evaluate(() => window.__forceApply('module', 'mod_blade', 1));
...
for (let i = 0; i < 6; i += 1) {
  ...
  console.log(`t=${i}: enemies=${s.enemies}`);
  await page.screenshot({ path: dir + `blade-${i}.png` });
}
await page.mouse.up();
